...
function Cart() {
    ...
    useEffect(() => {
        if (isCheckAll) {
            let total = listCarts.reduce((acc, item) =>
        ...
        <>
            <div className={cx('wrapper')}>
                ...
                <div className={listCarts.length > 0 ? cx('cart_content') : cx('hidden')}>
                                ...
                                localstorage.get('carts').map((cart, index) => (
                                    <li key={index} className={cx('product_item')}>
                                        <p className={cx('check')}>
                                            <input type="checkbox" checked={listCheck[index]} onChange={(e) => handleCheckItem(e, cart, index)} />
                                        </p>
                                        ...
                                        <a href='#' className={cx('product_info')}>
                                            ...
                                            <div className={cx('product_price')}>
                                                <p className={cx('price')}>{
                                                    numeral(cart.product.price).format('0,0[.]00 VNĐ')
                                                } đ</p>
                                                <p className={cart.product.price === cart.product.old_price ? cx('hidden') : cx('old_price')}>{
                                                    numeral(cart.product.old_price).format('0,0[.]00 VNĐ')
                                                } đ</p>
                                            </div>
                                        </a>
                                        <div className={cx('quantity')}>
                                            <div className={cx('quantity_btn')}>
                                                <p onClick={() => handleMinus(cart, index)} className={cx('btn_minus')}>
                                                    <FontAwesomeIcon icon={faMinus} />
                                                </p>
                                                <input type="number" value={cart.quantity} className={cx('quantity_num')} />
                                                <p onClick={() => handlePlus(cart, index)} className={cx('btn_plus')}>
                    ...
                    <div className={cx('right', 'hide-on-mobile-tablet')}>
                        ...
                        <Link to='/checkout' state={{ 'listCheckouts': listCheckouts }} className={cx('btn_check_out')}>
                            <Button primary disabled={totalPrice <= 0}>THANH TOÁN</Button>
                        </Link>
                    </div>
                </div>
                <div className={cx('payment_info_on_mobile_tablet', 'hide-on-pc')}>
                    ...
                    <Link to='/checkout' state={{ 'listCheckouts': listCheckouts }} className={totalPrice > 0 ? cx('btn_checkout_on_mobile_tablet') : cx('btn_checkout_on_mobile_tablet', 'disable')}>
                        Thanh toán
                ...
                <div className={listCarts.length === 0 ? cx('no_product_in_cart') : cx('hidden')}>
                    <img src='https://cdn0.fahasa.com/skin//frontend/ma_vanese/fahasa/images/checkout_cart/ico_emptycart.svg' alt='không có hình' />
