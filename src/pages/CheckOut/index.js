...
function CheckOut() {
    ...
    const listCheckouts = location.state.listCheckouts
                    ...
                    if (result.status == 'OK') {
                        ...
                        orderItems.forEach((orderItem) => {
                            fetch(`${api}/orderitems/insert`, {
                                ...
                                .then(result => {
                                    if (result.status == 'OK') {
                                        const carts = localstorage.get('carts')
    ...
    useEffect(() => {
        const address = `${watch('address')}, ${watch('wards')}, ${watch('districs')}, ${watch('city')}, ${watch('country')}`
    ...
    useEffect(() => {
        fetch(`${apiMaps}directions/v5/mapbox/driving/${locationShop[0]},${locationShop[1]};${locationAddress[0]},${locationAddress[1]}.json?access_token=${API_KEY}`)
            .then((response) => response.json())
            .then((result) => {
            ...
            <form onSubmit={handleSubmit(submit)}>
                <div className={cx('shipping_address')}>
                    ...
                    <div className={cx('form_group')}>
                        <div className={cx('form_content')}>
                            ...
                            <div className={errors.name ? cx('form_input', 'error') : cx('form_input')}>
                                ...
                                <p onClick={() => handleClearError('name')} className={errors.name ? cx('error_btn') : cx('hidden')}>
                    ...
                    <div className={cx('form_group')}>
                        <div className={cx('form_content')}>
                            ...
                            <div className={errors.email ? cx('form_input', 'error') : cx('form_input')}>
                                ...
                                <p onClick={() => handleClearError('email')} className={errors.email ? cx('error_btn') : cx('hidden')}>
                                    <FontAwesomeIcon icon={faCircleXmark} />
                                </p>
                            </div>
                        </div>
                    ...
                    <div className={cx('form_group')}>
                        <div className={cx('form_content')}>
                            ...
                            <div className={errors.phone ? cx('form_input', 'error') : cx('form_input')}>
                                ...
                                <p onClick={() => handleClearError('phone')} className={errors.phone ? cx('error_btn') : cx('hidden')}>
                                    <FontAwesomeIcon icon={faCircleXmark} />
                                </p>
                            </div>
                        </div>
                    ...
                    <div className={cx('form_group')}>
                        <div className={cx('form_content')}>
                            ...
                            <select
                                {...districsController.field}
                                disabled={!watch('city')}
                                className={cx('select')}>
                                {!watch('city') ? <option value="" selected disabled hidden>Vui lòng chọn tỉnh/thành phố trước</option> :
                                    <option value="" selected disabled hidden>Chọn</option>}
                                {
                                    listDistrics.map((distric, index) => (
                                        <option key={index} value={distric.name}>{distric.name}</option>
                                    ))
                                }
                            </select>
                        </div>
                    ...
                    <div className={cx('form_group')}>
                        <div className={cx('form_content')}>
                            ...
                            <select
                                {...wardsController.field}
                                disabled={!watch('districs')}
                                className={cx('select')}>
                                {!watch('districs') ? <option value="" selected disabled hidden>Vui lòng chọn quận/huyện trước</option> :
                                    <option value="" selected disabled hidden>Chọn</option>}
                                {
                    ...
                    <div className={cx('form_group')}>
                        <div className={cx('form_content')}>
                            ...
                            <div className={errors.address ? cx('form_input', 'error') : cx('form_input')}>
                                ...
                                <p onClick={() => handleClearError('address')} className={errors.address ? cx('error_btn') : cx('hidden')}>
                                    <FontAwesomeIcon icon={faCircleXmark} />
                                </p>
                            </div>
                        </div>
                ...
                <div className={cx('payment_method')}>
                    ...
                    <div className={cx('payment_content')}>
                        <input
                            ...
                            type='radio' id='shipping_method'
                            value='Giao hàng tiêu chuẩn' />
                        <label for='shipping_method' className={cx('payment_label')}>Giao hàng tiêu chuẩn: {numeral(shippingCost).format('0,0[.]00 VNĐ')} đ</label>
                    </div>
                ...
                <div className={cx('payment_method')}>
                    ...
                    <div className={cx('payment_content')}>
                        <input
                            ...
                            type='radio' id='payment_method'
                            value='Thanh toán bằng tiền mặt khi nhận hàng' />
                        <label for='payment_method' className={cx('payment_label')}>Thanh toán bằng tiền mặt khi nhận hàng</label>
                    </div>
                    <p className={cx('form_error', 'error_radio')}>{errors.payment_method?.message}</p>
                </div>
                ...
                <div className={cx('payment_method')}>
                    ...
                    <div className={cx('payment_content')}>
                        ...
                        <label for='note' className={cx('note_label')}>Ghi chú</label>
                        <div className={errors.message ? cx('form_input', 'input_note', 'error') : cx('form_input', 'input_note')}>
                            <input
                            ...
                            />
                            <p onClick={() => handleClearError('message')} className={errors.message ? cx('error_btn', 'btn_message') : cx('hidden')}>
                                <FontAwesomeIcon icon={faCircleXmark} />
                            </p>
                        </div>
                ...
                <div className={cx('summary')}>
                    <div className={cx('top', 'top_total')}>
                        <div className={cx('total_price_order')}>
                            ...
                            <p className={cx('top_price', 'price_top_total')}>{watch('shipping_method') ? numeral(price + shippingCost).format('0,0[.]00 VNĐ') : numeral(price).format('0,0[.]00 VNĐ')} đ</p>
                        </div>
                    </div>
            ...
            <div className={cx('payment_method')}>
                            ...
                            <div className={cx('product_content')}>
                                ...
                                <div className={cx('product_price')}>
                                    <p className={cx('price')}>{
                                        numeral(item.product.price).format('0,0[.]00 VNĐ')
                                    } đ</p>
                                    <p className={item.product.price === item.product.old_price ? cx('hidden') : cx('old_price')}>{
                                        numeral(item.product.old_price).format('0,0[.]00 VNĐ')
                                    } đ</p>
                                </div>
                ...
                <div className={cx('top')}>
                    ...
                    <div className={cx('shipping_cost')}>
                        <p className={cx('top_label')}>Phí vận chuyển {watch('shipping_method') ? `(${watch('shipping_method')})` : ''}</p>
                        <p className={cx('top_price')}>{watch('shipping_method') ? numeral(shippingCost).format('0,0[.]00 VNĐ') : 0} đ</p>
                    </div>
                    <div className={cx('shipping_cost')}>
